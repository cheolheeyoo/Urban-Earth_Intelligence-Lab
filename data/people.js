/* UEI Lab — bilingual content. Edit ko/en values here. */
window.UEI = window.UEI || {};
UEI.people = {
  "pi": {
    "name": {
      "ko": "유철희",
      "en": "Cheolhee Yoo"
    },
    "position": {
      "ko": "조교수",
      "en": "Assistant Professor"
    },
    "photo": "assets/img/people/cheolhee-yoo.png",
    "email": "cheolhee.yoo@pusan.ac.kr",
    "office": "",
    "cv": "",
    "bio": {
      "ko": "<p>2026년 3월 부산대학교에 부임했습니다. 이전에는 NASA 제트추진연구소(JPL)의 박사후연구원과 홍콩이공대학교 연구조교수로 재직했습니다. 2022년 UNIST에서 도시환경공학 박사학위를 받았습니다.</p>",
      "en": "<p>I joined Pusan National University in March 2026. Previously, I was a JPL Postdoctoral Fellow at NASA’s Jet Propulsion Laboratory and a Research Assistant Professor at The Hong Kong Polytechnic University. I received my Ph.D. in Urban &amp; Environmental Science and Engineering from UNIST in 2022.</p>"
    },
    "education": [
      {
        "year": {
          "en": "2017–2022",
          "ko": "2017–2022"
        },
        "text": {
          "ko": "도시환경공학 박사 · UNIST",
          "en": "Ph.D., Urban & Environmental Science and Engineering · UNIST"
        }
      },
      {
        "year": {
          "en": "2013–2017",
          "ko": "2013–2017"
        },
        "text": {
          "ko": "도시환경공학 학사 · UNIST · 최우등 졸업",
          "en": "B.S., Urban & Environmental Science and Engineering · UNIST · Summa Cum Laude"
        }
      }
    ],
    "experience": [
      {
        "year": {
          "ko": "2026.03–현재",
          "en": "Mar 2026–present"
        },
        "text": {
          "ko": "조교수 · 부산대학교 스마트시티전공",
          "en": "Assistant Professor · Smart City Convergence Major, Pusan National University"
        }
      },
      {
        "year": {
          "ko": "2026.07–08",
          "en": "Jul–Aug 2026"
        },
        "text": {
          "ko": "방문연구원 · 미국 뉴욕대학교",
          "en": "Visiting Scholar · New York University, USA"
        }
      },
      {
        "year": {
          "ko": "2025–2026",
          "en": "2025–2026"
        },
        "text": {
          "ko": "박사후연구원 · NASA 제트추진연구소(JPL)",
          "en": "Postdoctoral Fellow · NASA Jet Propulsion Laboratory (JPL)"
        }
      },
      {
        "year": {
          "ko": "2022–2025",
          "en": "2022–2025"
        },
        "text": {
          "ko": "연구조교수 · 홍콩이공대학교 토지측량 및 지리정보학과",
          "en": "Research Assistant Professor · Department of Land Surveying and Geo-Informatics, The Hong Kong Polytechnic University"
        }
      },
      {
        "year": {
          "ko": "2022.04–09",
          "en": "Apr–Sep 2022"
        },
        "text": {
          "ko": "박사후연구원 · 홍콩이공대학교",
          "en": "Postdoctoral Fellow · The Hong Kong Polytechnic University"
        }
      },
      {
        "year": {
          "ko": "2019.01–03",
          "en": "Jan–Mar 2019"
        },
        "text": {
          "ko": "연구 인턴 · 일본 RIKEN AIP",
          "en": "Research Intern · RIKEN AIP, Japan"
        }
      }
    ],
    "affiliation": {
      "ko": "부산대학교 스마트시티전공 (도시공학과 대학원)",
      "en": "Smart City Convergence Major, Graduate Program in Urban Engineering, Pusan National University"
    },
    "editorial": [
      {
        "year": {
          "ko": "현재",
          "en": "Current"
        },
        "text": "GIScience & Remote Sensing (IF: 6.8) · Associate Editor"
      },
      {
        "year": {
          "ko": "현재",
          "en": "Current"
        },
        "text": "ISPRS Journal of Photogrammetry and Remote Sensing (IF: 12.9) · Editorial Board Member"
      },
      {
        "year": {
          "ko": "현재",
          "en": "Current"
        },
        "text": "Humanities and Social Sciences Communications · Editorial Board Member"
      }
    ]
  },
  "groups": [
    {
      "title": {
        "ko": "학부생 인턴",
        "en": "Undergraduate interns"
      },
      "members": [
        {
          "name": {
            "ko": "학부생 인턴 01",
            "en": "Undergraduate Intern 01"
          },
          "role": {
            "ko": "학부생 인턴",
            "en": "Undergraduate intern"
          },
          "placeholderLabel": "01",
          "placeholder": true,
          "photo": "",
          "email": "",
          "interests": ""
        },
        {
          "name": {
            "ko": "학부생 인턴 02",
            "en": "Undergraduate Intern 02"
          },
          "role": {
            "ko": "학부생 인턴",
            "en": "Undergraduate intern"
          },
          "placeholderLabel": "02",
          "placeholder": true,
          "photo": "",
          "email": "",
          "interests": ""
        },
        {
          "name": {
            "ko": "학부생 인턴 03",
            "en": "Undergraduate Intern 03"
          },
          "role": {
            "ko": "학부생 인턴",
            "en": "Undergraduate intern"
          },
          "placeholderLabel": "03",
          "placeholder": true,
          "photo": "",
          "email": "",
          "interests": ""
        }
      ]
    }
  ],
  "alumni": []
};
